Meteor.methods({
    ...
    updateQuestionFromVolunteerMiniForm: function (options) {
        ...
        if (!question.answeredBy) { throw new Meteor.Error(404, "Question with id " + options.questionId + " does not have an answer yet."); }
        ...
        if (Meteor.isServer) {
            if (options.title && options.title.length > 0 && !question.slug) {
                updateDoc['$set']['slug'] = DigitalLeksehjelp.generateUniqueSlug(options.title);
            }
        }
